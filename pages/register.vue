    ...
    <div class="sign-up-container">
      <el-form ref="userForm" :model="params">
        <el-form-item class="input-prepend restyle" prop="nickname" :rules="[{
          ...
          <div>
            <el-input type="text" placeholder="你的昵称" v-model="params.nickname"></el-input>
        ...
        <el-form-item class="input-prepend restyle no-radius" prop="code" :rules="[{
          ...
          <div class="btn" style="position:absolute;right: 0;top: 6px;width: 40%;">
            <a href="javascript:" type="button" @click="getCodeFun()" :value="codeTest" style="border: none;background-color: cyan">{{codeTest}}</a>
          </div>
        </el-form-item>
      ...
      <div class="more-sign">
        <h6>社交帐号直接注册</h6>
        <ul>
          <li><a id="weixin" class="weixin" target="_blank" href="http://huaan.free.idcfengye.com/api/ucenter/wx/login">
      ...
      methods: {
        ...
        submitRegister() {
          this.$refs['userForm'].validate((valid) => {
            if (valid) {
              registerApi.submitRegisterInfo(this.params).then(response => {
                //提示注册成功
                this.$message({
                  type: 'success',
                  message: "注册成功"
        ...
        checkPassword(rule,value,callback){
          ...
          if (value.length <= 6 && value.length >20){
            return callback(new Error("密码不能少于6位,且不能超过20位"))
          }
          if (!(/^(?=.*([a-zA-Z].*))(?=.*[0-9].*)[a-zA-Z0-9-*/+.~!@#$%^&*()]{6,20}$/.test(value))){
            return callback(new Error('输入的密码必须是数字和字母，可以包含字符'))
          }
          return callback();
        }
      }
